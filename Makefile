...
migrate-create: ## Write the next migration pair, e.g. `make migrate-create name=users_table`
	@test -n "$(name)" || { echo "usage: make migrate-create name=<snake_case_name>"; exit 1; }
	cd $(CORE) && go run ./cmd/migrate create $(name)
...
.PHONY: verify
verify: ## Demonstrate the SHIP-1..9 acceptance criteria end to end
