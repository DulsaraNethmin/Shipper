...
| SHIP-115a | Delivery read shelf — a job's parties read its delivery detail and milestone history, auth class `RequireUser` | 3 | Both parties to a job read the driver assignment and every recorded milestone under the `/v1/jobs/{id}/delivery/` shelf; a stranger gets exactly what a missing job gets | SHIP-111, SHIP-115 |
...
**SHIP-115a takes the `/v1/jobs/{id}/delivery/` shelf SHIP-115 opened, and must stay on it.** `GET /v1/jobs/{id}/delivery` on its own is four segments and collides with `GET /v1/jobs/open/{id}` — `ServeMux` panics at registration, so the service does not start rather than answering a 404 somebody debugs. Five segments or more are safe, which is why SHIP-115's proof read is `GET /v1/jobs/{id}/delivery/proof` and why this one goes beside it rather than above it. `Docs/11` §9 carries the structural fix — moving the open feed off the `{id}` slot — and it is not this ticket's.
