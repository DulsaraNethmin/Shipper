...
// rejectCompeting closes every offer on the job that is still live (SHIP-93).
//
// Docs/02 §3: "awarding a job atomically marks one bid accepted and all others closed."
// [postgresStore.acceptBid] is the one; this is the others, and they are one statement apart inside
// one transaction because the sentence says *atomically*.
//
// # `Rejected`, and the vocabulary had already chosen it
//
// Docs/02 §4 gives eight statuses and exactly one of them means "the customer said no": `Rejected`,
// "an offer the customer declined". Awarding somebody else **is** declining every other offer, and it
// is the customer who did it — including a customer's own outstanding counter, which they decline by
// awarding elsewhere. The other three closed statuses each name a different event and would be a lie
// about how the offer ended: `Superseded` is displacement by a counter, `Withdrawn` is the provider's
// own act, and `Expired` is the offer's own terms running out. [StatusRejected]'s comment has named
// this ticket since SHIP-84, and `ck_bids_status` is unchanged — there is no migration here and
// nothing to widen.
//
// # `status = 'Submitted'` is the whole of "every other bid", and a constraint is why
//
// It reads as a narrowing and it is not: 000502's `ck_bids_superseded_is_not_live` makes the live
// offer and the head of a chain the same row by construction, so **there is no live offer this
// predicate misses**. What it excludes is every row that has already closed — a displaced offer at
// `Superseded`, a withdrawn one, an expired one — and excluding them is the point rather than a side
// effect. Overwriting `Superseded` with `Rejected` would replace the record of *how* an offer ended
// with the record of *when*, and Docs/01 §4.3 requires the platform to record every offer,
// counter-offer and withdrawal while Docs/02 §4 keeps the chain readable to both parties. **No
// constraint would refuse that write** — `ck_bids_superseded_is_not_live` permits `Rejected` on a
// displaced row — so this predicate is the only thing standing between the sweep and a rewritten
// history, which is why there is a test for each closed status rather than one for the shape.
//
// # The accepted bid is excluded by having already been accepted, and that is deliberate
//
// There is no `id <> $accepted` here, and leaving it out is a decision rather than an oversight.
// [Service.AwardBid] runs this at step 4, *after* the accept, so the awarded row is at `Accepted` and
// outside this predicate already. Adding the identifier would make the statement safe under an
// ordering it is not meant to run in — and would make moving it in front of the accept a change no
// test could see. Without it, a sweep that ran first would close the row the accept is about to name,
// `acceptBid`'s compare-and-set would match nothing, and the award would fail loudly and roll back.
