...
const _job = '0198f2c1-6b40-7a11-9c3e-2f9a4d51b7e0';
...
/// Signs a customer in and walks them to the offers on [_job] **the way a person gets there** — the
/// job list, the job, and the button on it.
///
...
Future<void> openOffers(
  ...
  if (jobs.page.data.isEmpty) {
    jobs.page = ApiPage<Job>(data: <Job>[jobs.detail(_job)]);
  }
  ...
  await tester.tap(find.byKey(const Key('job-$_job')));
  await tester.pumpAndSettle();
