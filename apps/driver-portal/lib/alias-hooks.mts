...
/**
 * The `@/` path alias, resolved for `node --test` (SHIP-120 follow-up).
 *
 * `tsconfig.json` maps `@/*` to the application root and the bundler honours it, but Node does not
 * read a `tsconfig`. So `lib/one-job.test.ts` — which imports the route handler in order to assert
 * on the request it sends upstream — cannot load a file that says `@/lib/link` without this.
 *
 ...
 * lines of resolution is the whole cost of the portal's outbound hop being under test at all; the
